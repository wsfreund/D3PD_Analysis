...
float Neural::propagate(const std::vector<float> &inputVector) const{
    for(unsigned int i=0; i<inputVector.size();i++)
        mM[0][i]=inputVector[i];
    for(unsigned int l=0; l<nodes.size()-1;l++){
        for(unsigned int k=0; k<nodes[l+1]; k++){
            mM[l+1][k]=bM[l][k];
            for (unsigned int j=0;j<nodes[l]; j++){
                mM[l+1][k]+=mM[l][j]*wM[l][k][j];
            }
            mM[l+1][k]=tanh(mM[l+1][k]);
        }
    }
